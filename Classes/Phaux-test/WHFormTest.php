...
class WHFormTest extends WHComponent {
	...
	public function renderContentOn($html){
		...
		$cont .= $html->form()->class("pure-form")->with(
		$html->fieldset()->with(
			$html->label("First Name:")->setInput(
			$html->textInput()->value($this->firstName)->callback($this,'setYourName')).
			$html->br().
			$html->label("Last Name:")->setInput(
			$html->textInput()->value($this->lastName)->callback($this,'setLastName')).
			$html->br().
			$html->label("Sex:")->setInput(
			$this->renderSexSelectOn($html)).
			$html->br().
			$html->submitButton()->
					class('pure-button pure-button-primary')->callback($this,'submitButton')->value("Submit!")			
			));
	
		$cont .= $html->form()->class("pure-form")->enctype("multipart/form-data")->with(
				$html->bold('File Test:').
				$html->br().
				$html->fileInput()->callback($this,'fileHandle').
				$html->br().
				$html->submitButton()->class('pure-button pure-button-primary')->callback($this,'submitButton')->value("Submit!")
				).
