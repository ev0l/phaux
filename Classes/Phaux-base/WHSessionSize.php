...
class WHSessionSize extends WHComponent {
	...
	*/
	...
	public function processObject($object){
		...
		if(!isset($this->summaryTable[get_class($object)])){
			$this->summaryTable[get_class($object)] = array();
		}
		if(!isset($this->summaryTable[get_class($object)]['total'])){
			$this->summaryTable[get_class($object)]['total'] = 0;
		}
		++$this->summaryTable[get_class($object)]['total'];

		//Only really works with object that define objectVars
		if(method_exists($object,'objectVars')){
			$things = $object->objectVars();
		}else{
			$things = get_object_vars($object);
		}
		foreach($things as $var => $value){
			...
			if(is_object($value)){
				...
				if(!isset($this->summaryTable[get_class($object)]['size'])){
					$this->summaryTable[get_class($object)]['size'] = 0;
				}
				$this->summaryTable[get_class($object)]['size'] += $this->sizeForVar($value);
			}			
		}
	...
	public function processArrayForObject($array,$object){
		foreach($array as $var => &$value){
			$this->summaryTable[get_class($object)]['size'] += $this->sizeForVar($var);
			if(is_array($value)){
				$this->processArrayForObject($value,$object);
